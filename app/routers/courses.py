...
def read_course(course_id: UUID, session: DBSession) -> CourseReadSingle:
    match session.get(Course, course_id):
        case None:
...
def read_course(course_id: UUID, session: DBSession) -> list[TopicReadList]:
    match session.get(Course, course_id):
        case None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Course not found")
        case course:
...
def read_course(course_id: UUID, topic_id: UUID, session: DBSession) -> list[TopicReadList]:
    match session.get(Course, course_id):
        case None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Course not found")
        case course:
